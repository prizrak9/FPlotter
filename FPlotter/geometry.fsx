...
module Plot =
    ...
    type PlotObj =
        ...
        | Surface of list<list<Vector>>
        | Mesh of list<Vector> * list<list<int>>
        static member points =
            function
            | Line a | Segment a | Segment3d a | Spline a | Spline3d a | Plane (a,_) | Mesh (a,_) -> a
            | Surface a -> [for a in a do yield! a]
...
let l4() =
    ...
    let buildSurface f uRange vRange = 
        [for u in uRange do
            for v in vRange -> 
                f u v]
